...
import '../progressIndicator/custom_progress_indicator_for_pagination.dart';
import '../style/color.dart';
import '../text/base_text.dart';
import 'ka_bottom_sheet.dart';

class SelectItemWithPaginationBottomSheet<T> extends StatelessWidget {
  final String title;
  final List<String> itemsLabel;
  final List<T> itemValues;
  final T selectedItemValue;
  final Rx<PageEntity> pageModel;
  final ScrollController scrollController;
  final Function(T item, String label) selectItem;

  const SelectItemWithPaginationBottomSheet(
      {Key? key,
      ...
      required this.scrollController, required this.pageModel})
      : assert(itemsLabel.length == itemValues.length),
        super(key: key);
      ...
      children: [
        ...
        Container(
          ...
          child: CustomListViewSeparator(
            ...
            itemCount: pageModel.value.itemCount,
            shrinkWrap: true,
            ...
            itemBuilder: (BuildContext context, int index) {
              ...
              if (page.paginateLoading &&
                  page.itemCount - 1 == index) {
                return const CustomProgressIndicatorForPagination();
              }
              return ItemWithPaginationBottomSheetCard<T>(
...
class ItemWithPaginationBottomSheetCard<T> extends StatelessWidget {
  final bool isSelected;
  final Function(T, String) onPress;
  final String item;
  final T value;

  const ItemWithPaginationBottomSheetCard({
    Key? key,
